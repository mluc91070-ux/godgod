...
3. **API + database + worker — Render.** `render.yaml` is a blueprint: New →
   Blueprint → point at this repository. It creates the free PostgreSQL, the web
   service, and an hourly cron that runs one observation pass and one research
   cycle. `ADMIN_TOKEN` is generated by Render so no human picks it. Set
   `CORS_ORIGINS` to the frontend URL once step 4 gives you one.
